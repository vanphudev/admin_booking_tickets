...
export function MultiTabsProvider({ children }: PropsWithChildren) {
   ...
   useEffect(() => {
      ...
      if (!isExisted) {
         setTabs((prev) => [...prev, { ...currentRouteMeta, key, children, timeStamp: getTimeStamp() }]);
      }
   ...
   const defaultValue: MultiTabsContextType = useMemo(
      ...
      [activeTabRoutePath, closeAll, closeLeft, closeOthersTab, closeRight, closeTab, refreshTab, tabs],
   );
